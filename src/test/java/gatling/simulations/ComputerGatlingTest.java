...
/**
 * Performance test for the Computer entity.
 */
public class ComputerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all computers").get("/api/computers").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new computer")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"introduced\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"removed\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"hardware\": 0" +
                                ", \"software\": 0" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_computer_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created computer").get("${new_computer_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created computer").delete("${new_computer_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Computer entity").exec(scn);
