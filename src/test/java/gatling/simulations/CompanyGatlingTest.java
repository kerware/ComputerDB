...
public class CompanyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all companies").get("/api/companies").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created company").get("${new_company_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created company").delete("${new_company_url}").headers(headers_http_authenticated))
                .pause(10)
        );
